...
export default function ChargerControl() {
  ...
  const fetchMeterValues = async (chargerId: number) => {
    try {
      const data = await getChargerMeterValues(chargerId);
      setMeterValues(data);
    } catch (err: any) {
      console.error(
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const response = await startCharging(selectedId);
  ...
  const handleStop = async () => {
    ...
    try {
      setActionLoading(true);
      const response = await stopCharging(selectedId);
      const updated = response.charger;
      setChargers((prev) =>
        prev.map((item) =>
          item.charger_id === updated.charger_id ? updated : item,
    ...
    <main className="p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Start or stop charging and view simulated OCPP meter values.
        </p>
      </div>
